...
import org.zonales.entities.OldPost;
 ...
 * @author nacho
 */
public final class ZSolrUpdateZone extends BaseService {

    private static CommonsHttpSolrServer server;
    private Gson gson;
    ZoneDao zoneDao;

    public ZSolrUpdateZone(String solrUrl) throws MalformedURLException {
        setServer(solrUrl);
        gson = new Gson();
    }

    public ZSolrUpdateZone() {
        gson = new Gson();
    }
    ...
    public void indexPost(List<String> ids) throws SolrServerException, IOException {
        ...
        for (SolrPost solrPost : rsp.getBeans(SolrPost.class)) {
            ...
            try {
                postIn = gson.fromJson(solrPost.getVerbatim(), Post.class);
            } catch (JsonParseException ex) {
                oldPost = gson.fromJson(solrPost.getVerbatim(), OldPost.class);
            }
    ...
    public void oldPostToPost(Post newPost, OldPost post) {
        newPost.setId(post.getId());
        newPost.setDocType("post");
        String zone = post.getZone();
        Zone newZone = new Zone();
        if (zone != null) {

            org.zonales.tagsAndZones.objects.Zone zoneObj = zoneDao.retrieve(zone.toLowerCase().replace(" ","_"));
            newZone.setId(String.valueOf(zoneObj.getId()));
            newZone.setName(zoneObj.getName());
            newZone.setType(zoneObj.getType().getName());


            newPost.setZone(newZone);
        }        
        newPost.setCreated(post.getCreated());
        newPost.setFromUser(post.getFromUser());        
        newPost.setModified(post.getModified());
        newPost.setRelevance(post.getRelevance());        
        newPost.setSource(post.getSource());        
        newPost.setText(post.getText());
        newPost.setTitle(post.getTitle());        
        newPost.setTags(post.getTags());
        newPost.setVerbatim(gson.toJson(post, Post.class));
    }
    ...
    public void serve(HttpServletRequest req, HttpServletResponse resp, Properties props) throws ServletException, IOException, Exception {
        ...
        try {
            ...
            setServer(solrURL);
            Logger.getLogger(ZSolrServer.class.getName()).log(Level.INFO, "URL SOLR: {0}", solrURL);
