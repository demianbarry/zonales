...
final public class Rule$horas extends Rule {

    private Rule$horas(String spelling, ArrayList<Rule> rules) {
    ...
    public static Rule$horas parse(ParserContext context) {
        context.push("horas");
        ...
        rule = null;
        if (parsed) {
            rule = new Rule$horas(context.text.substring(s0, context.index), e0);
        } else {
            context.index = s0;
        }
