...
final public class Rule$listaNegraPalabras extends Rule {

    private Rule$listaNegraPalabras(String spelling, ArrayList<Rule> rules) {
    ...
    public static Rule$listaNegraPalabras parse(ParserContext context) {
        context.push("listaNegraPalabras");
        ...
        rule = null;
        if (parsed) {
            rule = new Rule$listaNegraPalabras(context.text.substring(s0, context.index), e0);
        } else {
            context.index = s0;
        }
