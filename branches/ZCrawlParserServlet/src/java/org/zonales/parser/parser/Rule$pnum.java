...
final public class Rule$pnum extends Rule
{
  private Rule$pnum(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$pnum parse(ParserContext context)
  {
    context.push("pnum");
    ...
    rule = null;
    if (parsed)
      rule = new Rule$pnum(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("pnum", parsed);

    return (Rule$pnum)rule;
  }
}
