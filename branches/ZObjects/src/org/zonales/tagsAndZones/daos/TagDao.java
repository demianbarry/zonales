...
public class TagDao extends BaseDao {
    ...
    public Tag retrieve(String name) {
        Logger.getLogger(this.getClass().getName()).log(Level.SEVERE, "Buscando tag por nombre: {0}", new Object[]{name});
        BasicDBObject query = new BasicDBObject("name", name);
        return getTag(query);
    }
    ...
    private Tag getTag(BasicDBObject query) {
        ...
        Logger.getLogger(this.getClass().getName()).log(Level.SEVERE, "Tag encontrado: {0}", new Object[]{resp});
        if (resp == null) {
            return null;
        }
        resp.removeField("_id");
