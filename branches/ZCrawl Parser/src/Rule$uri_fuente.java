...
{
  private Rule$uri_fuente(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$uri_fuente parse(ParserContext context)
  {
    ...
    {
      {
        ...
        if (parsed)
        {
          ...
          {
            rule = Terminal$NumericValue.parse(context, "%x27", "[\\x27]", 1);
            if ((f1 = rule != null))
            {
              e1.add(rule);
              c1++;
    ...
    rule = null;
    if (parsed)
      rule = new Rule$uri_fuente(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;
