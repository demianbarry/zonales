 ...
 *
  ...
  * @subpackage Component
 */
class CustompropertiesModelCphierarchicfield extends JModel {
	/**
	* Field id
	*
	* @var integer
	*/
	...
	function & getData() {
		...
		if (empty ($this->_data)) {
			$database = $this->_db;
			/* retrieve record */
			$query = 'SELECT * FROM #__custom_properties_fields' . ' WHERE id = ' . $this->_id . ' AND parent_id != null ORDER BY ordering ';
