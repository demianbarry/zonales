<?php
/**
* Fields PluginManager class
*
* @version		$Id: hook.php 1439 2009-08-16 12:41:13Z mostafa.muhammad $
* @package		Joomla
* @subpackage	JWF.core
...
class JWFHookPluginManager{
	...
	function invokeMethod( $name, $which, $params  ){
		...
		foreach( $this->loadedPlugins as $p ){
			...
			$className = 'JWFHookHandler_'.$p->id;
			//PHP 4 fix
			$methodExists = false;
	...
	function loadPlugins()
	{
		...
		$pluginPath = JWF_BACKEND_PATH.DS.'plugins'.DS.'hook_handlers'.DS;
	...
	function _loadPlugin( $id, $name ){
		...
		$plugin->xml  = $pluginPath.$id.DS.$id.'.xml';
	
		$this->loadedPlugins[$id] = $plugin;
				
		//Load language files
		$lang =& JFactory::getLanguage();
		$lang->load('hook.'.ucfirst($id),JWF_BACKEND_PATH,null,false);
