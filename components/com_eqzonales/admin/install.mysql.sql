...
CREATE  TABLE IF NOT EXISTS `#__eqzonales_banda` (
  ...
  `cp_value_id` INT NOT NULL COMMENT 'tag (value) asociado en la tabla jos_customproperties_values' ,
  ...
  UNIQUE INDEX `uq_atributo_eq` (`cp_value_id` ASC, `eq_id` ASC) ,
...
INSERT INTO `#__eqzonales_banda` (`id`,`valor`,`peso`, `cp_value_id`,`eq_id`, `default`, `active`)
	SELECT `e`.`id`, `e`.`valor`, `e`.`peso`, `e`.`cp_value_id`, `e`.`eq_id`, `e`.`default`, `e`.`active` FROM `#__eqzonales_banda_bak` `e`
	ON DUPLICATE KEY UPDATE `id` = `e`.`id`, `valor` = `e`.`valor`, `peso` = `e`.`peso`, `cp_value_id` = `e`.`cp_value_id`, `eq_id` = `e`.`eq_id`, `default` = `e`.`default`, `active` = `e`.`active`;
