INSERT INTO `#__zonales_menu_bak` (`id`,`menu_id`,`value_id`,`title`)
	SELECT `m`.`id`, `m`.`menu_id`, `m`.`value_id`, `m`.`title` FROM `#__zonales_menu` `m`
	ON DUPLICATE KEY UPDATE `id` = `m`.`id`, `menu_id` = `m`.`menu_id`, `value_id` = `m`.`value_id`, `title` = `m`.`title`;
