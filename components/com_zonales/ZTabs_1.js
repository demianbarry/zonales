...
document.write('<script type="text/javascript" src="ZContext.js"></script>');
document.write('<script type="text/javascript" src="ZIRClient.js"></script>');
function ZTabs(){
    this.nodeURL = 'http://192.168.0.2:4000';
    this.sources = new Array();    
    this.zoneInitiated = false;
    this.zUserGroups = new Array();
    this.postInterval = null;
    this.postsContainer = null;
    this.newPostsContainer = null;
    this.verNuevosButton = null;
    this.verMasButton = null;
    this.postsDiv = null;
    this.mapDiv = null;
    ...
    this.initVista=function initVista(zCtx){
        if(this.postsContainer)
            this.postsContainer.empty();
        if(this.newPostsContainer)
            this.newPostsContainer.empty();
        this.verNuevosButton.setStyle('display','none');
        ...
        if (zCtx.zTab != 'geoActivos') {
            ...
            this.verMasButton.setStyle('display','block');
            //loadPost(true);
            ...
            this.verMasButton.setStyle('display','none');
            initMapTab();
            $('enLaRed').set({
    ...
    this.initPost=function initPost() {
        if (tab != 'geoActivos' && this.postsContainer) {
    ...
    function initZonas(selZone) {
        if (!this.zoneInitiated) {
            this.zoneInitiated = true;
    ...
    function setZone(zoneExtended, zoneName, parentId, parentName) {
        ...
        this.zirClient.setFirstIndexTime(null);
        this.zirClient.setLastIndexTime(null);
        this.zirClient.setMinRelevance(null);
        this.zirClient.setSearchKeyword("");
        $('zonalesSearchword').value = "buscar...";
        if (tab != 'geoActivos' && tab != 'editor' && tab != 'list' && this.postsContainer && this.newPostsContainer) {
            this.postsContainer.empty();
            this.newPostsContainer.empty();
        }
    ...
    function loadPost(first){
        ...
        zirClient.loadSolrPost(tab, zcGetEfectiveZoneName(), false, function(jsonObj) {
            if(typeof jsonObj != 'undefined'){
                if(first){
                    updatePosts(jsonObj,this.postsContainer);
                    armarTitulo(tab);
                }
                else {
                    updatePosts(jsonObj,this.newPostsContainer);
                    if(this.newPostsContainer.childNodes.length > 0){
                        this.verNuevosButton.value= this.newPostsContainer.getChildren('div').length+' nuevo'+(this.newPostsContainer.getChildren('div').length > 1 ? 's' : '')+'...';
                        this.verNuevosButton.setStyle('display','block');
                    } else{
                        this.verNuevosButton.setStyle('display','none');
                    }
                }
    ...
    function loadMorePost(){
        ...
        zirClient.loadSolrPost(tab, zcGetEfectiveZoneName(), true, function(jsonObj) {
            if(typeof jsonObj != 'undefined') {
                ...
                updatePosts(jsonObj, this.postsContainer,true);
                zirClient.searching = false;
            } else {
    ...
    function verNuevos(){
        ...
        $$('div#newPostsContainer div.story-item').set({
            ...
            this.postsContainer.getElements('div[id^=si]').each(function(element) {
                if (element.id == post.id) {
                    this.postsContainer.removeChild(element);
                }
            });
            ...
            if (tab == "enlared" || tab == "noticiasenlared" || tab == "portada"){
                ...
                newPost.injectTop(this.postsContainer);
            /*this.newPostsContainer.getElements('span.story-item-real-modified-date').each(function(newCount){
                var insertado = false;
                newCountModifiedDate = new Date(newCount.innerHTML).getMilliseconds();
                this.postsContainer.getElements('span.story-item-real-modified-date').each(function(count){
                ...
                if(!insertado){
                    newCount.getParent().getParent().getParent().getParent().getParent().injectInside(this.postsContainer);
                }
            });*/
            }

            if (tab == "relevantes" || tab == "noticiasenlaredrelevantes"){
                this.newPostsContainer.getElements('span.zonales-count').each(function(newCount){
                    var insertado = false;
                    this.postsContainer.getElements('span.zonales-count').each(function(count){
        ...
        //this.postsContainer.set({ style: 'background:#FFFFFF'});
        this.verNuevosButton.setStyle('display','none');
        this.newPostsContainer.empty();
        refreshFiltro();
    // this.postsContainer.setStyle('backgroundColor','#FFFFFF');
    }
    ...
    function updatePosts(json, component, more) {
        ...
        if(!insertado){
            div_story_item.injectInside(this.postsContainer);
        }*/
