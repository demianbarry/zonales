...
public class Parser
{
  ...
  static public void main(String[] args) throws Exception
  {
    ...
    if (!ok)
    {
      Globals.out.println("error: " + error);
      Globals.out.println("usage: Parser [-rule rulename] [-trace] <-file file | -string string> [-visitor visitor]");
    }
    else
    {
      try
      ...
      {
        Globals.out.println("argument error: " + e.getMessage());
      }
      catch (IOException e)
      {
        Globals.out.println("io error: " + e.getMessage());
      }
      catch (ParserException e)
      {
        throw new Exception("Error al parsear la configuración de extracción (el * marca el comienzo del error): " + e.getMessage());
      }
      catch (ClassNotFoundException e)
      {
        Globals.out.println("visitor error: class not found - " + e.getMessage());
      }
      catch (IllegalAccessException e)
      {
        Globals.out.println("visitor error: illegal access - " + e.getMessage());
      }
      catch (InstantiationException e)
      {
        Globals.out.println("visitor error: instantiation failure - " + e.getMessage());
      }
    }
  }
         ...
         ParserException
  {
    ...
    else if (rulename.equalsIgnoreCase("listaNegraUsuarios")) rule = Rule$listaNegraUsuarios.parse(context);
    else if (rulename.equalsIgnoreCase("listaNegraPalabras")) rule = Rule$listaNegraPalabras.parse(context);
    else if (rulename.equalsIgnoreCase("minActions")) rule = Rule$minActions.parse(context);
