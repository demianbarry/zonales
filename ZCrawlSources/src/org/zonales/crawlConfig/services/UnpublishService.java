 ...
 * @author nacho
 */
public class UnpublishService extends BaseService {
    ...
    public void serve(HttpServletRequest request, HttpServletResponse response, Properties props) throws ServletException, IOException, Exception {
        ...
        try {
            ...
            Unpublisher unpublisher = null;
            ArrayList<Plugin> plugins = service.getPlugins();
            for (Plugin plugin : plugins) {
                if ("Unpublisher".equals(plugin.getType())) {
                    unpublisher = (Unpublisher) Class.forName(plugin.getClassName()).newInstance();
                    break;
                }
            }

            if (unpublisher != null) {                
                out.print(unpublisher.unpublish(zgram, zgramId, props));
            } else {
