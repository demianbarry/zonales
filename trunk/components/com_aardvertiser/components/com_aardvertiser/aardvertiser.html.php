...
<div align="center">
    ...
    function showAds($rows, $option, $currency) {
        JHTML::stylesheet("aardvertiser.css","components/com_aardvertiser/")
...
<div id="ads">
	  ...
	  <div style="width: 62%; margin: auto 0; float: left; padding: 15px 0 0 0;">
		  <h3>
        <a href="' . $link . '">' . $row->ad_name . ' - ' . $row->ad_state . '</a>
      </h3>
    </div>
    <div style="width: 38%; margin: auto 0; float: left;" class="ads">
		  <div align="right" class ="adsum">Item Price: ' . $currency . $row->ad_price . '<br />Item Location: ' . $row->ad_location . ' <br /><b>' . $row->ad_state . '</b></div>
		</div>
		</div>
		 ';
            }
...
<br /><a href="<?php echo $link; ?>">&lt; return to categories</a><br /><br />
        ...
        function showAd($row, $option, $currency, $font, $state_color, $detail_color, $find, $googapi, $map) {
    ...
    <div class="aard-ad-2">
        ...
        if (!$showemail == 0 ) {
            ...
            $findat = ("@");
                            $finddot = (".");
                            $length = strlen($email);
                            $dotpos = strrpos($email, $finddot);
                            $atpos = strpos($email, $findat);
                            $name = substr($email, 0, $atpos);
                            $domain = substr($email, $atpos + 1, $dotpos - $length);
                            $tld = substr($email, $dotpos + 1);
//End email stripping
                            ?>
                <style type="text/css">
                    #box1{
                ...
                <div id="box2" >
                    <script language="JavaScript"> <!--
                        ...
                        document.write ('lto:<?php echo $name; ?>')
                        document.write ('&#64;')
                        document.write ('<?php echo $domain; ?>.<?php echo $tld; ?>"><?php echo $name; ?>')
                        document.write ('&#64;')
                        document.write ('<?php echo $domain; ?>.<?php echo $tld; ?></A>')
                        // -->
                    </script>
            <?php }
        ?>
...
//Get Distances x y
        ...
        if ($usersub == '') {
            ...
            $out = '<font color="' . $detail_color . '">' . $adpost . ' to ' . $userpost . ' is ' . round($dist / 1000, 1) . 'km approx. ';
        ...
        if ($find == 1) {?><div align="left" ><b><font color="<?php echo $font; ?>">Distance Finder: </font></b>
            ...
            <?php  if ($_POST) {
                ...
                elseif ($map == 0) {
                        ...
                        echo '<a target="_blank" href="components/com_aardvertiser/aardmap.php?userpost='. $userpost . '&adpost=' . $adpost . '&key=' . $googapi . '">(directions)</a></font>';
                    }
                } else {
...
<form action="index.php?option=com_aardvertiser&task=save" enctype="multipart/form-data" onsubmit="return validate_form(this);" method="post" name="adminForm" id="adminForm">
    <fieldset class="adminForm">
        ...
        ?>
                ...
                <td>
                            <?php
        echo $editor->display( 'ad_desc', $row->ad_desc , '100%', '200', '40', '4');
        ?>
                </td>
            </tr><tr>
                <td width="153" align="right" class="key">
...
<script type="text/javascript">
	...
	<br /><table><tr><td bgcolor="#FFC18F">
	 ...
	 If you want to change the images on an ad, please resubmit the add and delete the old one.</td></tr></table><BR><BR><BR>';
        }
        include 'footer.php';
    }
}
