...
class HTML_classifieds {
    function showCategories($rows, $option, $today) {
        ...
        $user_id = $user->get( 'id' );
        if ($user_id == 0) {
            $state = 'Please <a href="index.php?option=com_user&view=zlogin">log in</a> to submit your own item.';
        }
        else {
...
<div align="center">
	...
	$script = JUri::root().'/administrator/components/com_aadvertiser/includes/aadvertiser.js';
	$document->addScript($script);
        $script = JUri::root().'/administrator/components/com_aadvertiser/includes/_class.noobSlide.js';
	$document->addScript($script);
...
<div id="ads">
            ...
            foreach ($rows as $row) {
                ...
                // Genero HTML para el carrusel de imágenes
                $box = '';
                $thumbs = '';
                $handles = '';
                foreach($row->images as $key => $image) {
                    ...
                    $box .= '<span><img src="'.$image->src.'" alt="Photo" /></span>';

                    if(count($row->images)-1 == $key)
                        $box .= '</div>';

                    if($key == 0)
                        $thumbs = '<div id="thumbs">
                                <div class="thumbs">';

                    $thumbs .= '<div><img src="'.$image->src.'" alt="Photo Thumb" /></div>';

                    if(count($row->images)-1 == $key)
                        $thumbs .= '</div>';

                    if(count($row->images)-1 == $key)
                        $thumbs .= '<div id="thumbs_mask"></div>';

                    if($key == 0)
                        $handles = '<p id="thumbs_handles">';
                ...
                echo '<div style="';
                if (!$catimg) {
                    echo 'padding-top:10px; height:41px; ';
                }
