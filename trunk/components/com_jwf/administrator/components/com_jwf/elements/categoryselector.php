	 ...
	 */
	 function fetchChildren( $category, $level ){
		...
		foreach( $category->children as $child ){
			$child->title = str_repeat("...", $level) . $child->title;
			$output .= JHTML::_('jwf.indentedLine',"$child->id : '{$child->title}',",5);
			if( count( $child->children ))$output .= $this->fetchChildren( $child, $level+1);
		}
		return $output;
	}
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		foreach( $categoryLists as $contentType => $data ){
			...
			foreach( $data as $id => $obj ){
				$output .= JHTML::_('jwf.indentedLine',"$obj->id : '{$obj->title}',",5);
				if( count($obj->children))$output .= $this->fetchChildren( $obj,1 );
			}
