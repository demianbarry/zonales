...
class JWFComponentHandler_ads  extends JWFComponentHandler{
	...
	function getCategories(){
		...
		if( $major == 1 && $minor == 5 ){
			$db  =& JFactory::getDBO();
			$sql = 'SELECT id, title FROM `#__aard_cats`';
			$db->setQuery( $sql );
	...
	function getEditLink( $id ){
		return JRoute::_("index.php?option=com_aardvertiser&task=edit&cid[]=$id");
	}
	...
	function getLatestRevisionId( $id ){
		...
		$sql = "SELECT MAX(`version`) FROM #__aard_ads";
		$db->setQuery( $sql );
	...
	function deleteOrphans(){
                /*
		$db  =& JFactory::getDBO();
		$sql = "DELETE FROM `#__jwf_steps` WHERE `iid` NOT IN (SELECT `id` FROM `#__aard_ads`)";
		$db->setQuery( $sql );
		$db->query();*/
	...
	function getItemRevision( $id, $version ){
		...
		$sql = "SELECT `ad_name`,`ad_desc` FROM #__aard_ads WHERE `id`=$id $versionWhere";
		$db->setQuery( $sql );
