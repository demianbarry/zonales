...
function showTags($ce, $item, $params, $append_to_meta=false) {
    ...
    if($can_edit) {
        $result .=
                "<div id=\"addTagsDiv$cid\" style=\"display: none; background: none repeat scroll 0% 0% #F8F8F1; position: absolute; margin: 5px; padding: 10px; border: 1px outset grey;\">"
                ."<div id=\"tagsSearchHeader$cid\">"
                .JText::_('Search tags:')
                ."<input id=\"tagsSearchField$cid\" onkeyup=\"searchTags(event, $cid);\">"
                ."<a style=\"float: right; cursor: pointer; padding-top: 5px; padding-left: 20px;\" onclick=\"showAddTagsDiv($cid)\">X</a>"
                ."</div>"
                ."<form id=\"addTagsForm$cid\" method=\"post\" action=\"index.php?option=com_customproperties&task=addTags\" name=\"adminForm\">
                        <input type=\"hidden\" name=\"ce_name\" value=\"content\" />
                        <input type=\"hidden\" name=\"cid\" value=\"$cid\" />
                        <div id=\"tagsSearchResults$cid\" style=\"padding: 10px;\"></div>
                        <div id=\"selectedTags$cid\" style=\"padding: 10px; display: none;\">
                            <span class=\"article_separator\"/>
                            <b><u>Seleccionados</u></b>
                        </div>
                        <input type=\"button\" id=\"addTagsButton$cid\" onclick=\"addTags(event,$cid)\" value=\"".JText::_('Add tags')."\" style=\"float: left; border:1px solid #CCCCCC; color:#666666; font-family:\"trebuchet MS\";font-size:11px;margin:0;padding:0;\"/>
                    </form>
                </div>";
    }
...
function getThumb($file, $image_dir,  $width, $heigth, $extra="", $aspect=true, $quality = '75', $thumb_dir=null, $debug=false) {
    ...
    if(empty($thumb_dir)) {
        $thumb_dir = 'components'.DS.'com_customproperties'.DS.'images';
    }
